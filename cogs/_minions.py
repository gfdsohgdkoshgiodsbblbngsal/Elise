...
class minions_cog(commands.Cog):
    ...
    async def minions_slash(self, interaction: discord.Interaction, *, username: Optional[str] = None,
                            profile: Optional[str] = None):
        if not bot_can_send(interaction):
            return await interaction.respose.send_message("You're not allowed to do that here.", ephemeral=True)
        await self.minions(interaction, username, profile)
    
    #===================================================================================================================================
        
    async def minions(self, interaction: discord.Interaction, provided_username: Optional[str] = None, provided_profile_name: Optional[str] = None) -> None:

        ########## One: Get the right profile and username
        player_data: Optional[dict] = await get_profile_data(interaction, provided_username, provided_profile_name)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.hypixel.net/skyblock/profile?key={API_KEY}&profile={player_data['profile_id']}") as resp:
                combined_data = await resp.json()
        ...
        if len(minions) == 0:
            return await error(interaction, "Error, this person has never crafted a mininon before!", "Are they the right player?")

        ########## Three: Get the bazaar data + override 4 common minions
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://api.hypixel.net/skyblock/bazaar") as resp:
                data = await resp.json()["products"]
        ...
        if len(ordered) == 0:
            return await error(interaction, f"Error, `{username}` has maxed all minions!", "I guess just wait for the next update?")
        ...
        embed.set_footer(text=f"Command executed by {interaction.user.display_name} | Community Bot. By the community, for the community.")
