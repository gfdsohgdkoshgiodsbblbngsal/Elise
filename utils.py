...
#=============================================================
# Errors
async def error(interaction: discord.Interaction, title: str, description: str) -> None:
    embed = discord.Embed(title=title, description=description, colour=0xe74c3c)
    embed.set_footer(text=f"Command executed by {interaction.author.display_name} | Community Bot. By the community, for the community.")
...
async def smarter_find_closest(ctx, user_input: Optional[str]) -> tuple[Optional[str], Union[Optional[dict], Optional[str]]]:
    ...
    #=============================
    # Nothing
    return await error(ctx, "No item, enchant or pet found with that name!", "If you're searching for a pet, please end your search with 'pet', and if you're searching for an ultimate enchantment, please include `ultimate`.") 
